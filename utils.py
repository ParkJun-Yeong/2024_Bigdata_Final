...
def test_single_volume(image, label, net, classes, multimask_output, patch_size=[256, 256], input_size=[224, 224],
                       test_save_path="/mnt/ssd01_250gb/juny/vfss/SAMed/output", case=None, z_spacing=1, split="test"):
    ...
    if image.shape[-3] == 3:
        ...
        slice = image
        c, x, y = slice.shape[-3], slice.shape[-2], slice.shape[-1]
        if x != input_size[0] or y != input_size[1]:
            slice = zoom(slice, (1, input_size[0] / x, input_size[1] / y), order=3)
            # label = zoom(label, (input_size[0] / x, input_size[1] / y), order=3)  # previous using 0
            label_low_res = zoom(label, (input_size[0] / x, input_size[1] / y), order=0)
        new_x, new_y = slice.shape[-2], slice.shape[-1]  # [input_size[0], input_size[1]]
        if new_x != patch_size[0] or new_y != patch_size[1]:
            slice = zoom(slice, (1, patch_size[0] / new_x, patch_size[1] / new_y), order=3)  # previous using 0, patch_size[0], patch_size[1]
        inputs = torch.from_numpy(slice).unsqueeze(0).int().cuda()
    ...
    if test_save_path is not None:
        ...
        wandb.log({
            split+' image': wandb.Image(image.transpose(1,2,0)),
            split+' prediction': wandb.Image(pred),
            split+' groundtruth' : wandb.Image(label),
        })
        # im = Image.fromarray(image.astype(np.float32))
